...
<div class="container mt-5">
    ...
    <form action="/pemilik/editAlat" method="post" enctype="multipart/form-data" style="background-color: white;box-shadow: 0px 8px 16px 0px rgba(0, 0, 0, 0.142);" class="p-5 mb-5">
        @csrf
        <div class="row">
            ...
            <div class="col-md-8 col-12 mt-2 mt-md-0">
                <select class="form-control" name="kategori">
                    ...
                    @if (!$kategori->isEmpty())
                        ...
                        <option value="{{$item->nama_kategori}}" {{ old('kategori') ?? $alat->first()->fk_id_ketegori == $item->id_kategori ? 'selected' : '' }}>{{$item->nama_kategori}}</option>
                        @endforeach
                    @endif
                </select>
            </div>
        ...
        <input type="hidden" name="pemilik" value="{{$alat->first()->fk_id_pemilik}}">
